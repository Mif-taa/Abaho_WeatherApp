export class WeatherUI {
    ...
    updateLabels() {
        const set = (id, key) => {
            ...
            if (el)
                el.textContent = this.langService.get(key);
        };
        ...
        if (input)
            input.placeholder = this.langService.get('searchPlaceholder');
    ...
    showError(type) {
        ...
        if (el) {
            ...
            if (msgEl) {
                if (type === 'NOT_FOUND')
                    msgEl.textContent = this.langService.get('errorNotFound');
                else if (type === 'NETWORK_ERROR')
                    msgEl.textContent = this.langService.get('errorNetwork');
                else
                    msgEl.textContent = this.langService.get('errorGeneric');
            }
        }
    }
    showWeather(data, alert) {
        this.hideAll();
    ...
    renderWeather(data) {
        ...
        const icon = this.elements['weather-icon'];
        if (icon) {
        ...
        if (bar) {
            ...
            bar.style.width = `${pct}%`;
            bar.style.background = data.temperature >= 40
    ...
    toggleDarkMode() {
        ...
        if (btn)
            btn.textContent = isDark ? this.langService.get('lightMode') : this.langService.get('darkMode');
    ...
    restoreDarkMode() {
        if (localStorage.getItem('weather_dark') === '1') {
            ...
            const btn = this.elements['dark-toggle'];
            if (btn)
                btn.textContent = this.langService.get('lightMode');
        }
    }
    hideAll() {
